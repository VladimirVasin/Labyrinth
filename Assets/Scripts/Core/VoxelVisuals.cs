...
namespace Labyrinth.Core
{
    public static partial class VoxelVisuals
    {
        ...
        public static void ApplyMaterialProfile(Material material, Color color)
        {
            ...
            {
                material.SetFloat("_Smoothness", 0.015f);
            }
            ...
            {
                material.SetColor("_SpecColor", new Color(0.02f, 0.02f, 0.018f, color.a));
            }
            ...
            Func<int, int, int, bool> includeBlock = null)
        {
            ...
            var includedBlocks = new bool[safeBlocksX, safeBlocksY, safeBlocksZ];
            for (var x = 0; x < safeBlocksX; x++)
            {
                for (var y = 0; y < safeBlocksY; y++)
                {
                    ...
                    {
                        includedBlocks[x, y, z] = (includeBlock == null || includeBlock(x, y, z))
                            && !IsHiddenInteriorBlock(x, y, z, safeBlocksX, safeBlocksY, safeBlocksZ);
                    }
                }
            }
        ...
        {
            ApplyStaticMazeLightingProfile(renderer, false);
        }

        public static void ApplyStaticMazeLightingProfile(Renderer renderer, bool castRealtimeShadows)
        {
            if (renderer == null)
            {
                return;
            ...
            renderer.shadowCastingMode = castRealtimeShadows ? ShadowCastingMode.On : ShadowCastingMode.Off;
        }
        ...
        private static bool ShouldCastRealtimeShadow(string objectName, bool figurePart)
        {
            ...
            return figurePart
                ...
                || Contains(objectName, "House")
                || Contains(objectName, "Shop")
                || Contains(objectName, "Tavern")
                || Contains(objectName, "Farm")
                || Contains(objectName, "Hut")
                || Contains(objectName, "Guild")
                || Contains(objectName, "Infirmary")
                || Contains(objectName, "Forge")
                || Contains(objectName, "Market")
                || Contains(objectName, "Chapel")
                || Contains(objectName, "Antiquary")
                || Contains(objectName, "Mine")
                || Contains(objectName, "Tower")
                || Contains(objectName, "Roof")
                || Contains(objectName, "Wall")
                || Contains(objectName, "Door")
                || Contains(objectName, "Gate")
                || Contains(objectName, "Cart")
                || Contains(objectName, "Chest")
                || Contains(objectName, "Ore")
                || Contains(objectName, "Rock")
                || Contains(objectName, "Tree")
        ...
        public static Material GetVoxelLitMaterial(Material baseMaterial)
        {
            if (baseMaterial == null)
            {
                return null;
